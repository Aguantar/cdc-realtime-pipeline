...
def _report(**kw):
    ev = {'e': 'executionReport', 'E': 1789800000100, 's': 'BTCUSDT', 'c': 'ioc.20260919.BTCUSDT.3.12', 'S': 'BUY', 'o': 'LIMIT', 'f': 'IOC',
...
def test_cancel_report_keeps_original_client_id_and_dedup_keys():
    c = _report(x='CANCELED', X='CANCELED', c='cancel-req-id', C='mkr.20260919.BTCUSDT.1.5', I=9010)
    assert T.order_row_from_report(c, 'maker-probe')['client_order_id'] == 'mkr.20260919.BTCUSDT.1.5'
    assert T.strategy_of('mkr.20260919.BTCUSDT.1.5') == 'maker-probe'
    cid = T.client_order_id('unwind', 'BTCUSDT', 12, 1789800000)
    assert cid.startswith('unw.20260919.BTCUSDT.12.') and len(cid) <= 36 and T.strategy_of(cid) == 'unwind'

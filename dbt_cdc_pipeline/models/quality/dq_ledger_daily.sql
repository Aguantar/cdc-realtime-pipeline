...
ch_orders AS (
    SELECT toDate(fromUnixTimestamp64Milli(created_ms)) AS day_utc, symbol, count() AS ch_orders, countIf(status = 'FILLED') AS ch_filled,
...
ch_fills AS (
    SELECT toDate(fromUnixTimestamp64Milli(filled_ms)) AS day_utc, symbol, count() AS ch_trades, sum(qty) AS ch_trade_qty

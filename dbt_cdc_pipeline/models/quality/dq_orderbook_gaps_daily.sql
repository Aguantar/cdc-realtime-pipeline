...
WITH days AS (
    SELECT DISTINCT toDate(recv_ts) AS day
...
per_sec AS (
    ...
    FROM {{ source('raw', 'orderbook_raw') }}
    WHERE toDate(recv_ts) IN (SELECT day FROM days)
    GROUP BY s
),
first_ts AS (
...
grid AS (
    SELECT d.day, toDateTime(d.day) + toIntervalSecond(n.number) AS s
    FROM days AS d CROSS JOIN numbers(86400) AS n
    WHERE toDateTime(d.day) + toIntervalSecond(n.number) < now() - INTERVAL 120 SECOND   -- 아직 도착 중인 마지막 2분은 세지 않는다
      AND toDateTime(d.day) + toIntervalSecond(n.number) >= (SELECT s0 FROM first_ts)
),
joined AS (
    SELECT g.day, g.s, coalesce(p.c, 0) AS c
    FROM grid AS g LEFT JOIN per_sec AS p ON p.s = g.s
),
zero_runs AS (
    SELECT day, min(s) AS gap_start, count() AS gap_seconds
    FROM (
        SELECT day, s, s - toIntervalSecond(row_number() OVER (PARTITION BY day ORDER BY s)) AS grp
        FROM joined WHERE c = 0
    )
    GROUP BY day, grp
),
day_median AS (
    SELECT day, median(c) AS median_per_sec FROM joined GROUP BY day
),
day_stats AS (
    SELECT j.day, any(dm.median_per_sec) AS median_per_sec, countIf(j.c = 0) AS zero_seconds,
           countIf(j.c > 0 AND j.c < 0.2 * dm.median_per_sec) AS low_seconds, sum(j.c) AS snapshots
    FROM joined AS j INNER JOIN day_median AS dm ON dm.day = j.day
    GROUP BY j.day
),
gaps AS (
    SELECT day, count() AS gap_windows, sum(gap_seconds) AS gap_seconds_total, max(gap_seconds) AS gap_longest_s,
           groupArray(10)(concat(toString(gap_start), ' +', toString(gap_seconds), 's')) AS gap_sample
    FROM zero_runs WHERE gap_seconds >= 3 GROUP BY day
)
...
FROM day_stats AS ds LEFT JOIN gaps AS g ON g.day = ds.day

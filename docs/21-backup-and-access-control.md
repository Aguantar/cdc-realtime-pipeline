...
#### 5-12. 결정 수정 — 닫는 대신 **문을 단다** (09-20 10:25 UTC)
...
| 층 | 무엇 | 역할 |
|---|---|---|
| ① Caddy `basic_auth` | `scripts/ops/caddy-grafana-auth.sh` (sudo) | 인터넷에서 오는 사람을 문 앞에서 막는다. 보여줄 사람에게는 이 자격증명 하나만 준다 |
| ② Grafana 익명 Viewer | 문을 통과한 사람은 두 번 로그인하지 않는다 | 데모 경험을 매끄럽게 |
| ③ 데이터소스 계정 `grafana_public` | **보여주기로 한 범위 그 자체** | cdc_pipeline 읽기 + 대시보드가 실제 쓰는 system 표 6개. 쓰기 없음, `circuit_connect`(게임 닉네임 780행) 없음 |

**③이 핵심이다.** ①·②는 "누가 보는가"를 정하고, ③은 "무엇까지 보이는가"를 정한다.
문을 달아도 통과한 사람이 `/api/ds/query` 로 임의 SELECT 를 할 수 있으므로, 계정이 곧 공개 범위다.

**`readonly=1` 은 주지 않는다**: 그렇게 만들었더니 Grafana ClickHouse 플러그인이 세션 설정을 걸다 **500** 이 났다.
쓰기 차단은 GRANT 로 이미 충분하다(실측: `cdc_pipeline` 조회 200, `circuit_connect` 조회 **code 497**).

**순서가 중요하다**: Caddy 문을 먼저 달고 → 그다음 익명을 켠다. 반대로 하면 그 사이가 완전 공개다.
그래서 지금은 익명 **off** 상태로 두고 Caddy 단계를 기다린다.

#### 5-13. 적용 결과 — 비밀번호 하나로 데모, 범위는 계정이 정한다 (09-20 09:50 ~ 10:40 UTC)

| 층 | 적용 | 검증 |
|---|---|---|
| Caddy `basic_auth` | `demo` / 사용자 지정 | 무인증 **401**, 틀린 비밀번호 **401**, 맞으면 통과 |
| Grafana 익명 Viewer | on (문 뒤에서만) | 방문자는 **비밀번호를 한 번만** 친다 |
| 데이터소스 `grafana_public` | cdc_pipeline + 대시보드가 쓰는 system 표 6개 | 게임 DB 조회 **code 497**, 쓰기 **code 497** |
| 대시보드 노출 | 다른 프로젝트 2개 숨김 | 방문자 **4개**(CDC 3 + Airflow), 관리자는 전부 |

**Grafana 11 OSS 의 함정**: 폴더 권한에서 Viewer 를 빼도 **익명 사용자에게는 적용되지 않았다**
(폴더로 옮기고 권한을 고쳐도 익명이 6개를 다 봤고, 재시작해도 같았다).
**대시보드 단위 권한은 먹는다** — 같은 payload 를 `/api/dashboards/id/{id}/permissions` 로 주니 즉시 403·검색에서 제외.
숨긴 대시보드는 그 프로젝트(`~/circuit-connect/grafana/`)에 정의가 따로 있고 관리자에게는 그대로 보이므로 잃은 것이 없다.

**왜 이 구조인가 한 줄로**: 문(Caddy)은 *누가* 들어오는지, 익명은 *편의*, 계정은 *무엇까지* 보이는지를 정한다.
셋 중 하나라도 빠지면 "보여주되 그 이상은 안 준다"가 성립하지 않는다.

**남은 다듬기(급하지 않음)**: grafana 블록에 `header_up -Authorization` 이 없어 데모 자격증명이 Grafana 까지 전달된다
(airflow 블록에는 있다). 지금은 Grafana 에 `demo` 사용자가 없어 무시되고 익명으로 떨어지므로 동작에 문제는 없다.

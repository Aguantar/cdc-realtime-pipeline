...
> 진행 상황(09-20 06:30 UTC): **#1~#10 전부 완료**·커밋·푸시.
...
| 10 | 백업 | Binance 표가 자동 포함 | **판단 수정**: 기준은 거래소 소속이 아니라 "다른 사본이 있는가" → 전환 롤백본 4개 제외, `binance_orderbook_raw` 는 오히려 유지. 리허설 스크립트화 | 증분 8.7GB → 5.0GB, 복원본 FINAL 집계가 원본과 일치 | **완료** 06:30 |
...
## 10-실행 (09-20 06:20 ~ 06:30 UTC) — 백업 제외 목록·복원 리허설

### 원래 계획을 실측으로 뒤집었다
계획은 "Binance 표가 자동 포함되니 제외 목록에 넣자"였다. 크기를 재 보니 전제가 틀렸다.

| 표 | 디스크 | 백업에 넣나 | 왜 |
|---|---|---|---|
| `orderbook_raw` | 4.78 GiB | **제외**(기존) | 같은 DAG 이 Parquet(zstd, 원격 120일)로 따로 내보낸다 |
| `crypto_trades_rmt` | 3.74 GiB | **제외**(신규) | 전환 롤백본. 살아 있는 표가 이미 백업에 있다 = 같은 데이터 두 번. 09-25 삭제 예정 |
| `crypto_trades_v2` | 3.85 GiB | **제외**(신규) | 같음. 09-26 삭제 예정 |
| `crypto_trades_float_bak` | 3.95 GiB | **제외**(신규) | 같음. 09-27 삭제 예정 |
| `binance_trades_float_bak` | 129 MiB | **제외**(신규) | 같음. 09-27 삭제 예정 |
| `binance_orderbook_raw` | 23 MiB | **유지** | Parquet 아카이브가 **없다** → 백업이 30일 TTL 밖의 유일한 사본. 23 MiB 라 비용이 사실상 0 |

**기준은 크기도, 거래소 소속도 아니라 "지금 복구에 쓸 수 있는 유일한 사본인가"** 다.
그 기준으로 보면 Binance 호가는 빼면 안 되고, 롤백본 넷은 빼야 한다.

### 실측 효과
09-20 증분이 **8.7 GB** 였던 이유가 이것이었다 — #5 Decimal 전환이 만든 롤백본 셋이 전부 새 파트라
증분에 통째로 들어갔다. 제외 목록을 적용해 같은 base(`full_20260917`)로 다시 만들었다.

| | 크기 | 소요 |
|---|---|---|
| 제외 전(`incr_20260920`) | 8.7 GB | — |
| 제외 후(같은 base) | **5.0 GB (−43%)** | 25초 |

목록은 `BACKUP_EXCLUDE` 상수에 **이유 주석과 함께** 두고, `_except()` 가 **실재하는 표만** 골라 넣는다
(롤백본은 곧 삭제되는데 없는 표를 적으면 그날 백업이 통째로 깨진다).

### 복원 리허설 — `scripts/ops/restore-rehearsal.sh`
docs/21 의 리허설(분기 1회 수동)을 스크립트로 만들었다. 실행하며 배운 것 셋:

**(가) 데이터베이스째 복원하면 안 된다.** `cdc_pipeline` 에는 Kafka 엔진 표가 6개 있다(`ledger_*_queue`).
백업 메타데이터에 실제로 들어 있는 것을 확인했다. 복원본이 뜨면 **같은 컨슈머 그룹으로 프로덕션 원장
메시지를 가로챈다.** 재해 복구라면 맞지만 살아 있는 클러스터에서 하는 리허설에서는 그 자체가 사고다.
→ 표 단위 복원으로 바꿨다.

**(나) pipeline 사용자로는 복원이 안 된다.** `ACCESS_DENIED: INSERT, CREATE TABLE ON cdc_restore_test`.
접근 통제(docs/21 §2)가 의도대로 막은 것이다. 복원은 관리자(default)로 한다 — 런북에 적지 않으면
사고 한복판에서 이걸 알아내게 된다.

**(다) 복원본을 FINAL 없이 읽으면 틀려 보인다.** 첫 대조에서 09-19 행 수가 3,489,987(복원본) vs
3,228,857(원본)로 달라 심장이 내려앉았는데, 원본 쪽에만 FINAL 을 걸었던 것이었다. 체결 표는
ReplacingMergeTree 이고 #6 재처리분 중복이 아직 머지 전이다. 양쪽 다 FINAL 로 읽으면 일치한다.

### 검증
| 항목 | 결과 |
|---|---|
| 복원 | 3표(체결·Binance 호가·마켓 상태 SCD) **18초** |
| 체결 행 수 | 복원본 117,273,698 = 백업 시각 기준 원본 117,273,698 (**정확히 일치**) |
| 09-19 집계(FINAL) | 행 3,228,857 / 금액 합 `2534251503935.06666635543932` — **원본과 소수 20자리까지 동일** |
| 마켓 상태 SCD | 289 = 289 |
| Binance 호가 | 221,861 vs 222,087 — 차이 226행은 백업 뒤 도착분(이벤트 시각은 백업 전) |
| 정리 | 시험 백업·복원본 삭제, 원장 경로 무영향 확인 |

**행 수가 아니라 금액 합까지 본 이유**: #5 의 타입 변환 사고가 행 수로는 통과하고 금액 합에서만 잡혔다.
"복구됐다"는 값이 같아야 하는 말이다.

### 곁가지로 찾은 것 — 조용한 실패처럼 보이는 정상
리허설 뒤 원장 경로를 확인하다가 가상 트레이더의 STATS 카운터가 **4시간 넘게 멈춰 있는 것**을 봤다
(`events=1849 orders=508` 고정, errors=0). 정지로 보고 조사했더니 01:40 에 열린 주문을 **전부 정상 취소**하고
끝낸 것이었고, 원인은 `MAX_ORDERS_PER_SYMBOL_PER_DAY=60` 상한이었다. 실제로 5심볼 모두 정확히 60건.
→ 장애가 아니지만 **장애와 구분이 안 되는 것이 문제**라, 상한에 닿을 때 이유를 로그로 남기게 했다
(심볼당 하루 한 번, UTC 자정에 리셋). 같은 조사를 다시 하지 않기 위해서다.

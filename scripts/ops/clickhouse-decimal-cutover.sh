...
#   CAST/toDecimal64/accurateCast 는 모두 **버림**이라 0.00518(Float64 = 0.00517999999…) 이 0.00517999 가 된다.
#   KRW-PEPE 한 행에서 금액이 42.86 KRW 어긋났고(차이 = volume × 1e-8), 월별 금액합이 2e-8 만큼 작게 나와 잡혔다 — 행수만 봤으면 못 봤다.
#   toString 은 그 double 로 되돌아가는 **가장 짧은 십진 표기**를 주므로 원본 DECIMAL(20,8) 을 그대로 복원한다(MySQL 원본과 대조 확인: 4285662548.26254840).
...
verify)
  ...
  ;;
cutover)
  # 전제: Flink CDC 잡이 **정지**된 상태(세이브포인트). 정지 중이라 잔여분 복사가 필요 없다.

...
#
# 왜: `grafana.calmee.store` 가 basic_auth 없이 localhost:3000 으로 넘어가고 있었다.
#   airflow·code 블록에는 있는 잠금이 grafana 블록에만 없어서, 대시보드와 /api/ds/query 가
#   인터넷 전체에 열려 있었다(실측 200). 지금은 Grafana 익명을 꺼서 막았지만,
#   그러면 **설정 하나가 유일한 잠금**이다. 앞단에도 문을 둔다(airflow 와 같은 방식).
#
...
echo
echo "적용됨. 확인:"
echo "  무인증:  curl -s -o /dev/null -w '%{http_code}\\n' https://grafana.calmee.store/   → 401 이어야"
echo "  인증:    curl -s -o /dev/null -w '%{http_code}\\n' -u '$USER_NAME:<비밀번호>' https://grafana.calmee.store/  → 200"
echo

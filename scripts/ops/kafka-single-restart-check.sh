...
for i in $(seq 1 40); do m=$(MY "SELECT count(*) FROM crypto_db.crypto_trades WHERE created_at >= '$LO' AND created_at < '$HI'"); c=$(CH "SELECT count() FROM cdc_pipeline.crypto_trades WHERE source_ts >= '$LO' AND source_ts < '$HI'"); say "  catch-up +$((i*15))s mysql=$m ch=$c connect=$(CSTATE)"; [ "$m" = "$c" ] && [ "$m" != "0" ] && break; sleep 15; done

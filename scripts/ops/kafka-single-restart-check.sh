...
for i in $(seq 1 40); do m=$(MY "SELECT count(*) FROM crypto_db.crypto_trades WHERE created_at >= '$LO' AND created_at < '$HI'"); c=$(CH "SELECT uniqExact(trade_id) FROM cdc_pipeline.crypto_trades WHERE source_ts >= '$LO' AND source_ts < '$HI'")   # 2026-09-18: count() 는 재전송 중복(239)으로 MySQL 과 안 맞아 상한까지 돎 → 고유 건수; say "  catch-up +$((i*15))s mysql=$m ch=$c connect=$(CSTATE)"; [ "$m" = "$c" ] && [ "$m" != "0" ] && break; sleep 15; done

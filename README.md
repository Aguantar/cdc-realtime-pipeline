...
![Grafana Dashboard](<img width="2527" height="1235" alt="image" src="https://github.com/user-attachments/assets/5bb28f6c-1998-4620-b9b6-851f925331b4" />
)
...
```
┌─────────────────┬─────────────────┬─────────────────┬─────────────────┬─────────────────┬─────────────────┐
│  Total Trades   │ Avg CDC Latency │  Active Alerts  │  Total Volume   │ Markets Tracked │Pipeline Status  │
│  (총 체결 건수)  │ (평균 지연시간)  │   (이상 탐지)    │(최근1시간 거래액)│ (모니터링 마켓)  │(파이프라인 상태) │
│   6,761,873     │    3.30 ms      │       0         │    ₩43.2B       │       5         │   🟢 LIVE       │
└─────────────────┴─────────────────┴─────────────────┴─────────────────┴─────────────────┴─────────────────┘
┌──────────────────────────────────────┬──────────────────────────────────────┐
│  Trade Volume (5분 총 거래금액)        │  Bid vs Ask (매수/매도 비율)           │
│  ───────────────────                 │  ██ BTC  ██ XRP  ██ ETH  █ SOL █DOG │
│  5개 코인 합산 라인 차트 (₩ 단위)       │  마켓별 매수/매도 건수 막대 차트        │
└──────────────────────────────────────┴──────────────────────────────────────┘
┌──────────────────────────────────────┬──────────────────────────────────────┐
│  BTC Price (실시간 BTC 가격)           │  CDC Latency (CDC 지연시간)            │
│  🔴 빨간 점선 = 이상 탐지              │  avg(녹색) / max(주황) 추이            │
│  price(녹) / low(노) / high(파)       │  목표: <10ms, 실측: ~3ms              │
└──────────────────────────────────────┴──────────────────────────────────────┘
┌─────────────────────────────────────────────────────────────────────────────┐
...
**상단 KPI (6개)** — 파이프라인 핵심 지표 한눈에

| 패널 | 데이터 범위 | 설명 |
|------|-----------|------|
| **Total Trades (총 체결 건수)** | 전체 누적 | 파이프라인 가동 이후 총 체결 건수 (현재 676만+) |
| **Avg CDC Latency (평균 지연시간)** | 최근 1시간 | MySQL → ClickHouse 평균 CDC 지연 (목표: <10ms, 실측: ~3ms) |
| **Active Alerts (이상 탐지)** | 최근 1시간 | 이상 탐지 룰에 걸린 알림 건수 |
| **Total Volume (최근 1시간 거래금액)** | 최근 1시간 | 5개 마켓 합산 체결 금액 (₩ 자동 포맷: K, M, B) |
| **Markets Tracked (모니터링 마켓)** | 고정값 | BTC, ETH, XRP, SOL, DOGE (5개) |
| **Pipeline Status (파이프라인 상태)** | 최근 5분 | 5분 내 데이터 유입 여부 (🟢 LIVE / 🔴 STALE) |

**중단 차트 (4개)** — 시장 흐름 + 이상 탐지 시각화

| 패널 | 위치 | 데이터 소스 | 설명 |
|------|------|-----------|------|
| **Trade Volume (5분 총 거래금액)** | 좌상 | `trade_aggregations` | 5개 코인 합산 거래금액 라인 차트 (₩ 단위, Flink 5분 윈도우 집계) |
| **Bid vs Ask (매수/매도 비율)** | 우상 | `trade_aggregations` | 마켓별 매수/매도 건수 막대 차트 (1시간 집계, 자동 갱신) |
| **BTC Price (실시간 BTC 가격)** | 좌하 | `crypto_trades` | 분 단위 평균/최저/최고 + 🔴 **이상 탐지 빨간 점선** (Grafana Annotation) |
